...
export default async function handler(request) {
  ...
  try {
    ...
    if (country) {
      const denmarkVariations = ['Denmark', 'denmark', 'DENMARK', 'Danmark', 'Kingdom of Denmark'];
      response.isDenmark = denmarkVariations.some(variation =>
        country.toLowerCase().includes(variation.toLowerCase())
      );
      
      if (response.isDenmark) {
